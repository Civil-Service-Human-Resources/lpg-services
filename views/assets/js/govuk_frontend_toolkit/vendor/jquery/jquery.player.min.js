	...
	var aq = 'undefined',
		...
		aK = (function () {
			...
			if (
				...
				(typeof aL.readyState == aq && (aL.getElementsByTagName('body')[0] || aL.body))
			) {
				aP()
			}
			if (!ak) {
...
window.NOMENSA.player.YoutubePlayer.prototype = {
	getYTOptions: function () {
		var b = this,
			a = {
				...
				events: {
					onReady: function (c) {
						b.$html.find('iframe').attr({id: b.config.id, role: 'presentation'})
						b.onPlayerReady(c)
					},
	...
	state: {ended: 0, playing: 1, paused: 2, unstarted: -1},
	...
	getFlashVars: function () {
		var a = {controlbar: 'none', file: this.config.media}
	...
	generateFlashPlayer: function (c) {
		...
		var h = {id: this.config.id, name: this.config.id}
		var e = $('<' + this.config.flashContainer + ' />')
			.attr('id', 'player-' + this.config.id)
			.addClass('flashReplace')
			.html(
				'This content requires Macromedia Flash Player. You can <a href="http://get.adobe.com/flashplayer/">install or upgrade the Adobe Flash Player here</a>.'
			)
		...
		setTimeout(function () {
			swfobject.embedSWF(
				...
				e.attr('id'),
				g.config.flashWidth,
				g.config.flashHeight,
				'9.0.115',
				null,
				a,
				f,
				h,
				g.config.swfCallback
			)
...
window.NOMENSA.player.MediaplayerDecorator.prototype.generatePlayerContainer = function () {
	var a = $('<' + this.config.playerContainer + ' />')
			...
			.click(function () {
				if ($(this).hasClass('play')) {
					$(this)
						.removeClass('play')
						.addClass('pause')
						.attr({title: 'Pause', id: 'pause-' + l.config.id})
						.text('Pause')
					l.play()
				} else {
					$(this)
						.removeClass('pause')
						.addClass('play')
						.attr({title: 'Play', id: 'play-' + l.config.id})
						.text('Play')
					l.pause()
				}
			})
...
window.NOMENSA.player.MediaplayerDecorator.prototype.getVolControls = function () {
	...
	var e = c
		.createButton('vol-down', '-<span class="ui-helper-hidden-accessible"> Volume Down</span>')
		.click(function () {
			c.voldwn()
		})
	var f = $('<span />')
		.attr({id: 'vol-' + c.config.id, class: 'vol-display'})
		.text('100%')
	var a = [b, e, h, f]
...
window.NOMENSA.player.MediaplayerDecorator.prototype.getSliderBar = function () {
	var c = $('<span />')
		...
		.html(
			'<p>The timeline slider below uses WAI ARIA. Please use the documentation for your screen reader to find out more.</p>'
		)
	var a = $('<span />')
...
window.NOMENSA.player.MediaplayerDecorator.prototype.updateSlider = function () {
	...
	$('#slider-' + this.config.id)
		...
		.attr({'aria-valuenow': b, 'aria-valuetext': b.toString() + ' percent'})
		.css('left', b.toString() + '%')
...
window.NOMENSA.player.MediaplayerDecorator.prototype.updateLoaderBar = function () {
	...
	$('#loaded-bar-' + this.config.id)
		.attr({'aria-valuetext': a.toString() + ' percent', 'aria-valuenow': a})
...
window.NOMENSA.player.MediaplayerDecorator.prototype.getControls = function () {
	...
	var d = $('<a />')
		.attr('href', 'http://www.nomensa.com?ref=logo')
...
;(function (a) {
	a.fn.player = function (k, f) {
		var e = {
			...
			playerSkip: 10,
			volumeStep: 10,
			buttons: {forward: true, rewind: true, toggle: true},
		...
		var d = {
			generatePlayerContainer: function () {
				var l = a('<' + this.config.playerContainer + ' />')
					.css(this.config.playerStyles)
					.addClass('player-container')
				if (window.NOMENSA.browser.msie) {
			...
			getFlashVars: function () {
				var l = {controlbar: 'none', file: this.config.media}
				if (this.config.image != '') {
			...
			generateFlashPlayer: function (n) {
				...
				var s = {id: this.config.id, name: this.config.id}
				var p = a('<' + this.config.flashContainer + ' />')
					.attr('id', 'player-' + this.config.id)
					.addClass('flashReplace')
					.html(
						'This content requires Macromedia Flash Player. You can <a href="http://get.adobe.com/flashplayer/">install or upgrade the Adobe Flash Player here</a>.'
				...
				setTimeout(function () {
					swfobject.embedSWF(
						...
						p.attr('id'),
						r.config.flashWidth,
						r.config.flashHeight,
						'9.0.115',
						null,
						l,
						q,
						s,
						r.config.swfCallback
					)
					if (b()) {
			...
			generateHTML5Player: function (m, p, o) {
				...
				n[0].className = 'video'
				var l = a('<' + p + ' />')
					.attr({id: this.config.id, src: this.config.media, type: o})
					.css({width: '100%', height: '50%'})
				if (a.trim(this.config.image) != '') {
					l.attr({poster: a.trim(this.config.image)})
				}
						...
						.click(function () {
							if (a(this).hasClass('play')) {
								a(this)
									.removeClass('play')
									.addClass('pause')
									.attr({title: 'Pause', id: 'pause-' + v.config.id})
									.text('Pause')
								v.play()
							} else {
								a(this)
									.removeClass('pause')
									.addClass('play')
									.attr({title: 'Play', id: 'play-' + v.config.id})
									.text('Play')
								v.pause()
							}
						})
			...
			getVolControls: function () {
				...
				var s = n
					.createButton('vol-up', '+<span class="ui-helper-hidden-accessible"> Volume Up</span>')
					.click(function () {
						n.volup()
					})
				var p = n
				...
				var q = a('<span />')
					.attr({id: 'vol-' + n.config.id, class: 'vol-display'})
					.text('100%')
				var l = [m, p, s, q]
				var o
			...
			updateSlider: function () {
				...
				a('#slider-' + this.config.id)
					.find('a.ui-slider-handle')
					.attr({'aria-valuenow': m, 'aria-valuetext': m.toString() + ' percent'})
					.css('left', m.toString() + '%')
				a('#progress-bar-' + this.config.id)
					.attr({'aria-valuenow': m, 'aria-valuetext': m.toString() + ' percent'})
			...
			updateLoaderBar: function () {
				...
				a('#loaded-bar-' + this.config.id)
					.attr({'aria-valuetext': l.toString() + ' percent', 'aria-valuenow': l})
