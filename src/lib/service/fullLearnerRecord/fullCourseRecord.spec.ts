...
function createDummyModule(moduleId: string, optional: boolean) {
	const mod = new Module(moduleId, 'Blended')
...
describe('Test the areAllModulesComplete function', () => {
	...
	it('Should return true when all required modules are completed', () => {
		...
		fullRecord.modules.set('MOD001', createDummyFullModuleRecord('MOD001', RecordState.Completed, true))
		fullRecord.modules.set('MDO002', createDummyFullModuleRecord('MDO002', RecordState.Completed, true))
	...
	it('Should return false when 1 required module is remaining', () => {
		...
		fullRecord.modules.set('MOD001', createDummyFullModuleRecord('MOD001', RecordState.Completed, true))
		fullRecord.modules.set('MDO002', createDummyFullModuleRecord('MDO002', RecordState.Completed, true))
		fullRecord.modules.set('MDO003', createDummyFullModuleRecord('MDO003', RecordState.InProgress, false))
	...
	it('Should return true when 2 optional modules are remaining', () => {
		const fullRecord = new FullCourseRecord(course, USERID, record)
		fullRecord.modules.set('MOD001', createDummyFullModuleRecord('MOD001', RecordState.NotStarted, true))
		fullRecord.modules.set('MDO002', createDummyFullModuleRecord('MDO002', RecordState.InProgress, true))
		const hasAllRequiredBeenCompleted = fullRecord.areAllRequiredModulesComplete()
		expect(hasAllRequiredBeenCompleted).to.equal(true)
	})
})
