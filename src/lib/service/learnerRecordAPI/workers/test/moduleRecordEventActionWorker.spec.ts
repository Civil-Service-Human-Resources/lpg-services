...
import {
    genericEvent, getCourseRecordWithOneModuleRecord, getCourseRecordWithTwoModuleRecords,
    getCourseWithOneRequiredModule, getCourseWithTwoRequiredModules,
    mockGetCourseRecord, mockTime, testDateAsStr, testUser, mockCreateCourseRecord, mockCreateModuleRecord, genericModuleRecord, getEventModuleRecord, mockPatchModuleRecord
...
describe('Should test the module event action worker classes', () => {
    ...
    describe('Should test skipping a booking', () => {
        ...
        it(`Should unregsiter the course record if it is currently registered`, async () => {
            const course = getCourseWithTwoRequiredModules()
            const courseRecord = getCourseRecordWithTwoModuleRecords(
                course.id, RecordState.Registered, course.modules[0].id, RecordState.InProgress,
                course.modules[1].id, RecordState.Completed
            )
            const event = genericEvent()
            const worker = new SkipBookingActionWorker(course, testUser, event, course.modules[1])
    ...
    describe('Should test completing an event module', () => {
        ...
        it(`Should complete the course record if it is currently approved`, async () => {
            const course = getCourseWithTwoRequiredModules()
            const courseRecord = getCourseRecordWithTwoModuleRecords(
                course.id, RecordState.Registered, course.modules[0].id, RecordState.Completed,
                course.modules[1].id, RecordState.Approved
            )
            console.log(course.id)
            const event = genericEvent()
            const completedModuleRecord = getEventModuleRecord(course.modules[1].id, course.id,
                RecordState.Completed, event.id)
            const worker = new CompleteBookingActionWorker(course, testUser, event, course.modules[1])
            await testUpdateCourseRecord(
                worker,
                courseRecord,
        ...
        it(`Should NOT update the module record if it has not been approved`, async () => {
            ...
            const courseRecord = getCourseRecordWithTwoModuleRecords(
                course.id, RecordState.Null, course.modules[0].id, RecordState.InProgress,
                course.modules[1].id, RecordState.Registered
            )
            const event = genericEvent()
            const worker = new CompleteBookingActionWorker(course, testUser, event, course.modules[1])
