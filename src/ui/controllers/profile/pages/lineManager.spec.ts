...
describe("Line manager middleware tests", () => {
	...
	const run = async (
		userLinemanagerEmail: string | undefined, lineManagerEmail: string | undefined,
		confirmLineManagerEmail: string | undefined, pageBehaviour: PageBehaviour = setupBehaviour) => {
	...
	describe("User setup tests", () => {
		it("Should update and redirect if there are no errors", async () => {
			await run(undefined, 'example@example.com',
				'example@example.com')
