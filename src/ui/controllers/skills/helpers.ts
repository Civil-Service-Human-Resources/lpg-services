...
export function formatAnswerSubmissionDate(answerSubmissions: AnswerSubmission[]) {
	...
	for (let answerSubmission of answerSubmissions) {
		answerSubmission.submittedDateString =
			answerSubmission.completedOn[2] + '/' + answerSubmission.completedOn[1] + '/' + answerSubmission.completedOn[0]

		answerSubmission.type = answerSubmission.type.charAt(0).toUpperCase() + answerSubmission.type.slice(1).toLowerCase()
	}
